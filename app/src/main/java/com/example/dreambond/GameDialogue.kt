...
// Character dialogue and text generation logic.
// Handles all dynamic replies, goodnight messages, and memory-based initiated lines.

fun generateDynamicReply(state: GameUiState, option: DialogueOption): String {
    ...
    if (characterName == "Alice") {
        return when (option.text) {
            ...
            "I was just curious." ->
                ...
                "I... I am still trying to understand you."
        }
    }

    // Mina character logic
    return when {
        affection < 10 -> when (option.text) {
            "I wanted to see you." -> {
            ...
            "I could not sleep." ->
                "Then maybe the night brought you here for a reason."

            else ->
                "You're a little hard to read... but I don't mind."
        }

        affection < 25 -> when (option.text) {
            "I wanted to see you." -> when {
            ...
            "I could not sleep." -> {
                ...
                "You always say things that make me curious."
        }

        affection < 50 -> when (option.text) {
            "I wanted to see you." -> {
                if (moodIntensity >= 2) {
            ...
            "I was just curious." -> {
                ...
                "You know... you're kind of cute when you act mysterious."
        }

        else -> when (option.text) {
            "I wanted to see you." -> {
                if (moodIntensity >= 2) {
...
fun generateGoodnightMessage(state: GameUiState): String {
    val memory = state.memory
    val mood = state.mood
    val moodIntensity = state.moodIntensity

    return when {
        memory.lastDateScene == "Tea House Date 🍵" ->
            "Goodnight... the tea house felt peaceful with you."

        memory.lastDateScene == "Bookstore Date 📚" ->
            "Goodnight... I keep thinking about that quiet bookstore."
        ...
        memory.favoriteTime == "Rain 🌧️" ->
            ...
            "Playful" -> {
                ...
                "Goodnight... I'll be waiting."
        }
    }
}

fun generateMemoryRecallLine(state: GameUiState, personality: String): String? {
    val memory = state.memory
    val characterName = state.selectedCharacter?.name
    val moodIntensity = state.moodIntensity
    ...
    if (characterName == "Alice") {
        return when {
            memory.lastDateScene == "Tea House Date 🍵" ->
                "I remembered the tea house... it felt quiet and gentle."

            memory.lastDateScene == "Bookstore Date 📚" ->
                "I remembered the bookstore... it felt peaceful being there with you."

            memory.favoriteFood.contains("Coffee", ignoreCase = true) ->
                "I remembered you like coffee... I thought that was kind of sweet."
...
fun generateMinaInitiatedLine(state: GameUiState, personality: String): String? {
    val memory = state.memory
    val mood = state.mood
    val moodIntensity = state.moodIntensity
    ...
    return when {
        ...
        memory.lastDateScene == "Tea House Date 🍵" ->
            "I thought about that tea house again... it felt calm being there with you."

        memory.lastDateScene == "Bookstore Date 📚" ->
            "I remembered the bookstore... it felt like time slowed down there."
